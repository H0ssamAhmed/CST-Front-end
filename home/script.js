...
export function createInfoElement(product) {
  ...
  if (product.discount > 0) {
    ...
    spanPriceDiscount.innerText = `$${product.price - ((product.price * product.discount) / 100).toFixed(2)
      }`;
...
function updateCountdown() {
  ...
  const productId = "1720647618965"; // Replace with dynamic value
...
export function getMessage() {
  ...
  const msgeIconHolder = document.querySelector(".msge-icon-holder")
  const massageCounter = document.getElementById("massageCounter")

  const msgbody = document.querySelector(".msge-body")
  const currentUser = JSON.parse(localStorage.getItem("currentUser"))
  const AllMessages = document.querySelector(".All-messages")
  let UserAnswers = (JSON.parse(localStorage.getItem('answers')) || [])?.filter((user) => user.email == currentUser.email)
  msgbody.style.width = "400px"
  msgbody.style.height = "100vh"
  msgbody.style.top = "0"
  msgbody.style.right = "-400px"
  msgbody.style.transition = "0.3s"
  msgbody.style.background = 'white'
  msgbody.style.zIndex = '500001'

  if (UserAnswers.length != 0) {
    massageCounter.innerHTML = UserAnswers.length
    AllMessages.innerHTML = ""
    for (let i = 0; i < UserAnswers.length; i++) {
      let ans = document.createElement("div")
      ans.style.background = '#cff4fc'
      ans.classList = "p-2 mb-3 d-flex justify-content-between align-items-start"
      ans.innerHTML = `
      <p>${UserAnswers[i].answer}</p>
      <button class="btn btn-danger" onclick="delteMsg(${UserAnswers[i].id})">Delete</button>
      `
      AllMessages.appendChild(ans)
    }
  }


  msgeIconHolder.classList = "d-flex flex-column justify-content-center text-center"
  msgeIconHolder.style.padding = "5px"

  msgeIconHolder.style.cursor = "pointer"
  massageIcon.addEventListener("click", function () {
    msgbody.style.right = "0px"
  })
...
window.delteMsg = (id) => {
  const currentUser = JSON.parse(localStorage.getItem("currentUser"))
  const restMsage = (JSON.parse(localStorage.getItem('answers')) || [])
    ?.filter((user) => user.email == currentUser.email)
    ?.filter((msg) => msg.id != id)
  localStorage.setItem('answers', JSON.stringify(restMsage))
  getMessage()
